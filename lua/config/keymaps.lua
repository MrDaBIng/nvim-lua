...
local mappings = {
    ...
    {from = ">", to = ":BufferLineCycleNext<CR>"},
    {from = "<", to = ":BufferLineCyclePrev<CR>"},
    ...
    {mode = mode_i, from = "<A-j>", to = "<Down>"},
    {mode = mode_i, from = "<A-k>", to = "<Up>"},
    {mode = mode_i, from = "<A-l>", to = "<Right>"},
    {mode = mode_i, from = "<A-f>", to = "<S-Left>"},
    {mode = mode_i, from = "<A-g>", to = "<S-Right>"},
    ...
    {from = '<F5>'  , to = function() require 'telescope'.extensions.dap.configurations {} end},
    {from = '<F10>' , to =  function() dap.step_over() end},
    {from = '<F11>' , to =  function() dap.step_into() end},
    {from = '<F12>' , to =  function() dap.step_out() end},
    {from = '<Leader>b',  to = function() dap.toggle_breakpoint() end},
    {from = '<Leader>B',  to = function() dap.set_breakpoint() end},
    {from = '<Leader>lp', to =  function() dap.set_breakpoint(nil, nil, vim.fn.input('Log point message: ')) end},
    {from = '<Leader>dr', to =  function() dap.repl.open() end},
    {from = '<Leader>dl', to =  function() dap.run_last() end},
    {from = '<Leader>dh', to = function() require('dap.ui.widgets').hover() end},
    {mode = mode_nv, from = '<Leader>dp', to = function() require('dap.ui.widgets').preview() end},
    {from = '<Leader>dh', to = function() require('dap.ui.widgets').hover() end},
    {mode = mode_nv, from = '<Leader>dp', to = function() require('dap.ui.widgets').preview() end},
    {from = '<Leader>ds', to = function()
      local widgets = require('dap.ui.widgets')
      widgets.centered_float(widgets.scopes)
    end},
    {from = '<Leader>df', to = function()
      local widgets = require('dap.ui.widgets')
      widgets.centered_float(widgets.frames) end},

    {from = "<leader>o", to = "<cmd>Lspsaga outline<CR>"},
    ...
    {from = "gh", to = "<cmd>Lspsaga finder<CR>"},
    {from = "gr", to = "<cmd>Lspsaga rename<CR>"},
    {from = "gD", to = "<cmd>Lspsaga peek_definition<CR>"},
    {from = "gd", to = "<cmd>Lspsaga goto_definition<CR>"},
...
for _, v in ipairs(mappings) do
    vim.keymap.set(v.mode or "n", v.from, v.to, {noremap=true, silent=true})
end
